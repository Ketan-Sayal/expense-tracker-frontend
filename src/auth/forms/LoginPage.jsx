...
export default function LoginPage() {
  ...
  const login = async(data)=>{
    // console.log(data);
    // {user:sendUser, acessToken}
    setError("");
    setLoading(true);
    try {
      const res = await axios.post("https://expense-tracker-backend-qwkl.onrender.com/users/login", data);
      const result = res.data?.data;
      if(result?.user._id){
        const token = result.acessToken;
        Cookies.set("token", token, {httpOnly:false, secure:true, sameSite:"Lax", expires:5});
        setUser(result.user);
        setIsLoggedIn(true);
        navigate("/");
      }else{
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async tokenResponse => {
      ...
      const res = await axios.post("https://expense-tracker-backend-qwkl.onrender.com/users/login/google-oauth", {username:name, email});
      const result =  res.data?.data;
      if(result?.user){
      Cookies.set("token", result.acessToken, {expires:5, httpOnly:false});
      setIsLoggedIn(true);
      setUser(result.user);
      navigate("/");
      }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          <div className="space-y-6">
            <form action="" onSubmit={handleSubmit(login)} className='w-full space-y-6'>
            <div>
            ...
            <button
              disabled={loading}
              type="submit"
              className="w-full bg-gradient-to-r from-purple-600 cursor-pointer to-violet-600 text-white py-3 px-4 rounded-lg font-medium hover:from-purple-700 hover:to-violet-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 transform transition duration-200 hover:scale-[1.02] active:scale-[0.98]"
            >
